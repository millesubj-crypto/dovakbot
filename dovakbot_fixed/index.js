...
import { scheduleDailyLottery, buyLottery } from './lottery.js';
  ...
  try {
    ...
    if (commandName === '슬롯') {
      ...
      if (bet <= 0 || bet > userData.balance) {
        await interaction.reply({ content: '❌ 베팅 금액 오류 (잔고 부족 또는 0 이하)', ephemeral: true });
        return;
      }
      ...
      if (cherryCount === 2) { reward -= 500; penaltyText = '💥 체리 2개! 500원 차감!'; }
      else if (cherryCount === 3) { reward -= 2000; penaltyText = '💀 체리 3개! 2000원 차감!'; }
      ...
      await interaction.reply({
        content:
          ...
          `💰 최종 잔고: ${balance.toLocaleString()}원\n` +
          `${reward > 0 ? `🎉 보상: +${reward.toLocaleString()}원` : reward < 0 ? `💸 손실: ${reward}원` : ''}`
      });
      return;
    }
    ...
    if (commandName === '경마') {
      ...
      if (!horseNum || horseNum < 1 || horseNum > 7) {
        await interaction.reply({ content: '❌ 말 번호는 1~7 사이로 입력하세요.', ephemeral: true });
        return;
      }
      ...
      await interaction.reply({ content: `🏁 경마를 시작합니다! ${bet.toLocaleString()}원 베팅 (말 번호 ${horseNum})` });
    ...
    if (commandName === '관리자지급') {
      ...
      await interaction.reply({ content: `✅ ${target.username}님에게 ${amount.toLocaleString()}원을 지급했습니다.`, ephemeral: true });
      return;
    }
    ...
    try {
      if (!interaction.replied && !interaction.deferred) {
        ...
        await interaction.editReply({ content: '⚠️ 오류가 발생했습니다.' });
      }
    } catch {}
  }
});
