...
client.on('interactionCreate', async (interaction) => {
  try {
    ...
    if (!userData || typeof userData.balance !== 'number') {
      await interaction.reply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription('⚠️ 유저 데이터를 불러오지 못했습니다.')], ...EPH });
      return;
    }
    ...
    if (commandName === '돈줘') {
      ...
      if (!(await canClaimDaily(user.id))) {
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.gray).setDescription('⏰ 이미 오늘의 기본금을 받았습니다.\n내일 다시 시도해주세요.')] });
        return;
      }
      const reward = 1000;
    ...
    if (commandName === '잔고') {
      ...
      await interaction.editReply({
        embeds: [new EmbedBuilder()
          ...
          .addFields({ name: '잔고', value: `**${userData.balance.toLocaleString()}원**` })],
      });
      return;
    }
    ...
    if (commandName === '슬롯') {
      ...
      let pnl = 0, resultLabel = '', embedColor = COLOR.gray;
      const bonusLines = [];
    ...
    if (commandName === '복권결과') {
      ...
      if (!(await isBotAdmin(user.id))) {
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription('❌ 봇 관리자 권한이 없습니다.')] });
        return;
      }
    ...
    if (commandName === '경마') {
      ...
      if (!horseNum || horseNum < 1 || horseNum > 7) {
        await interaction.reply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription('❌ 말 번호는 1~7 사이로 입력하세요.')], ...EPH });
        return;
      }
      if (!interaction.channel) {
        await interaction.reply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription('❌ 채널 정보를 불러올 수 없습니다.')], ...EPH });
        return;
      }
    ...
    if (commandName === '골라') {
      ...
      if (items.length < 2) {
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription('⚠️ 항목을 2개 이상 쉼표로 구분해서 입력해주세요.\n예: `짜장면, 짬뽕, 볶음밥`')] });
        return;
      }
    ...
    if (commandName === '봇관리자추가') {
      ...
      if (target.id === user.id) {
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription('⚠️ 자기 자신은 지정할 수 없습니다.')] });
        return;
      }
      if (await isBotAdmin(target.id)) {
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.gray).setDescription(`⚠️ **${target.username}**님은 이미 봇 관리자입니다.`)] });
        return;
      }
      await addBotAdmin(target.id, user.id);
      await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.green).setDescription(`✅ **${target.username}**님에게 봇 관리자 권한을 부여했습니다.`)] });
      return;
    }
    ...
    if (commandName === '봇관리자제거') {
      ...
      const target = options.getUser('대상');
      if (!(await isBotAdmin(target.id))) {
        await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.red).setDescription(`⚠️ **${target.username}**님은 봇 관리자가 아닙니다.`)] });
        return;
      }
      await removeBotAdmin(target.id);
      await interaction.editReply({ embeds: [new EmbedBuilder().setColor(COLOR.green).setDescription(`✅ **${target.username}**님의 봇 관리자 권한을 해제했습니다.`)] });
      return;
    }
    ...
    try {
      ...
      if (interaction.deferred || interaction.replied)
        ...
        await interaction.reply({ embeds: [errEmbed], ...EPH });
    } catch {}
  }
});
